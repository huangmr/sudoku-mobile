...
const LEVELS: { key: Level; label: string; emoji: string; desc: string }[] = [
  ...
  { key: 'medium', label: 'Medium', emoji: '🔥', desc: '27 clues • Steady challenge' },
  { key: 'hard',   label: 'Hard',   emoji: '⚡', desc: '22 clues • Serious thinking' },
  { key: 'horror', label: 'Horror', emoji: '💀', desc: '17 clues • Expert only' },
];
...
export default function HomeScreen() {
  ...
  const s = StyleSheet.create({
    header: { paddingTop: Platform.OS === 'ios' ? 60 : 24, paddingHorizontal: 20, paddingBottom: 16 },
      ...
      <View style={s.section}>
        ...
        {LEVELS.map(({ key, label, emoji, desc }) => (
          <TouchableOpacity
            key={key}
            style={s.levelBtn}
            onPress={() => router.push({ pathname: '/(tabs)/game', params: { level: key } })}
          >
