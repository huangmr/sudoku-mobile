...
export default function GameScreen() {
  ...
  const loadPuzzle = useCallback(async () => {
    ...
    startGame(p);
  }, [levelParam, puzzleIdParam, reset, startGame]);
  ...
  const handleCompletion = useCallback(async () => {
    ...
    router.push({
      ...
      params: {
        ...
        puzzleId: puzzle.puzzleId,
        ...(rewardReason
          ? { rewardReason, rewardDiamonds: String(rewardDiamonds) }
          : {}),
      },
    });
  }, [puzzle, elapsedSeconds, mistakes, isLoggedIn, user, updateUser]);

  const handleBuyLife = useCallback(() => {
    if (!isLoggedIn || (user?.diamonds ?? 0) < LIFE_COST) return;
    Alert.alert("Buy a Life", `Spend ${LIFE_COST} 💎 to gain an extra life?`, [
  ...
  }, [isLoggedIn, user, updateUser, addLife]);

  const loadPuzzleRef = useTrackedRef(loadPuzzle);
  useEffect(() => {
  ...
  const handleCompletionRef = useTrackedRef(handleCompletion);
  useEffect(() => {
    if (status === "completed") {
      handleCompletionRef.current();
    }
  }, [status, handleCompletionRef]);
