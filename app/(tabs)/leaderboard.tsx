...
import { getDailyLeaderboard, getAllTimeLeaderboard, LeaderboardEntry } from '@/api/leaderboard';
...
export default function LeaderboardScreen() {
  ...
  const s = StyleSheet.create({
    container: { flex: 1 },
    header: { paddingTop: Platform.OS === 'ios' ? 60 : 24, paddingHorizontal: 20 },
    ...
    rank: { width: 32, fontSize: 16, fontWeight: '700', color: colors.textSecondary },
  ...
  const renderItem = ({ item }: { item: LeaderboardEntry }) => {
    ...
    return (
      <View style={[s.row, { borderBottomColor: colors.border, backgroundColor: isSelf ? colors.primaryLight : 'transparent' }]}>
        <Text style={s.rank}>{medal ?? `#${item.rank}`}</Text>
        <Text style={s.name} numberOfLines={1}>{item.displayName}{isSelf ? ' (you)' : ''}</Text>
  ...
  return (
    <ThemedBackground style={s.container}>
      <View style={s.header}>
        ...
        <View style={s.tabs}>
          {(['daily', 'alltime'] as const).map(t => (
            <TouchableOpacity
              key={t}
              style={[s.tab, { borderColor: tab === t ? colors.primary : colors.border, backgroundColor: tab === t ? colors.primary : 'transparent' }]}
              onPress={() => setTab(t)}
            >
      ...
      {isLoading ? (
        ...
        <FlatList data={data ?? []} keyExtractor={i => String(i.rank)} renderItem={renderItem} />
      )}
    </ThemedBackground>
  );
}
