...
const REWARD_CONFIG: Record<
  RewardReason,
  { icon: string; title: string; subtitle: (d: number) => string }
> = {
  streak: {
    icon: "🔥",
    title: "Streak Reward!",
    subtitle: (d) =>
      `You're on a roll! You earned ${d} 💎 for your daily streak.`,
  },
  top_winner: {
    icon: "🏆",
    title: "Top Winner!",
    subtitle: (d) =>
      `You made the leaderboard! You earned ${d} 💎 for finishing in the top 10.`,
  },
  invitation: {
    icon: "🎁",
...
export function RewardPopup({ visible, reason, diamonds, onClose }: Props) {
  ...
  const sparkle = useMemo(() => new Animated.Value(0), []);
    ...
    >
      <View style={styles.overlay}>
        ...
        >
          ...
          >
            <Text style={[styles.diamondAmount, { color: colors.primary }]}>
              +{diamonds} 💎
            </Text>
          </View>

          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
            {cfg.subtitle(diamonds)}
          </Text>
